...
TEST_F(GaugeTest, TestGaugeName) {
  std::unique_ptr<GCMGauge<>> gauge(GCMGauge<>::New("name", "help"));
...
TEST_F(GaugeTest, TestGaugeLabelNames) {
  std::unique_ptr<GCMGauge<string, string, int>> gauge(
      GCMGauge<string, string, int>::New("name", "one", "two", "three",
                                         "help"));
...
TEST_F(GaugeTest, TestGaugeWithLabels) {
  std::unique_ptr<GCMGauge<std::string, int>> gauge(
      GCMGauge<std::string, int>::New("name", "a string", "an int", "help"));
